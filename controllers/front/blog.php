...
class Cs24blogblogModuleFrontController extends ModuleFrontController
{
    ...
    {
        include_once(_PS_MODULE_DIR_.'cs24blog/classes/captcha.php');
        $captcha = new Cs24Captcha();
    ...
    protected function comment()
    {
        ...
        $post['id_cs24_blog_blog'] = Tools::getValue('id_cs24_blog_blog');
        $post['submitcomment'] = Tools::getValue('submitcomment');

        if (!empty($post)) {
            $comment = new Cs24BlogComment();
    ...
    public function initContent()
    {
        ...
        if ($config->get('url_use_id', 1)) {
            // URL HAVE ID
            $blog = new Cs24BlogBlog(Tools::getValue('id'), $this->context->language->id);
        } else {
            // REMOVE ID FROM URL
        ...
        if (is_dir(_PS_THEME_DIR_.'modules/cs24blog/views/templates/front/'.$template) || is_dir(_PS_MODULE_DIR_.'cs24blog/views/templates/front/'.$template)) {
            if (file_exists(_PS_THEME_DIR_.'modules/cs24blog/views/templates/front/'.$template.'/_social.tpl') || file_exists(_PS_MODULE_DIR_.'cs24blog/views/templates/front/'.$template.'/_social.tpl')) {
                $_social = 'module:cs24blog/views/templates/front/'.$template.'/_social.tpl';
            } else {
                $_social = 'module:cs24blog/views/templates/front/default/_social.tpl';
            }

            if (file_exists(_PS_THEME_DIR_.'modules/cs24blog/views/templates/front/'.$template.'/_facebook_comment.tpl') || file_exists(_PS_MODULE_DIR_.'cs24blog/views/templates/front/'.$template.'/_facebook_comment.tpl')) {
                $_facebook_comment = 'module:cs24blog/views/templates/front/'.$template.'/_facebook_comment.tpl';
            } else {
                $_facebook_comment = 'module:cs24blog/views/templates/front/default/_facebook_comment.tpl';
            }

            if (file_exists(_PS_THEME_DIR_.'modules/cs24blog/views/templates/front/'.$template.'/_diquis_comment.tpl') || file_exists(_PS_MODULE_DIR_.'cs24blog/views/templates/front/'.$template.'/_diquis_comment.tpl')) {
                $_diquis_comment = 'module:cs24blog/views/templates/front/'.$template.'/_diquis_comment.tpl';
            } else {
                $_diquis_comment = 'module:cs24blog/views/templates/front/default/_diquis_comment.tpl';
            }

            if (file_exists(_PS_THEME_DIR_.'modules/cs24blog/views/templates/front/'.$template.'/_local_comment.tpl') || file_exists(_PS_MODULE_DIR_.'cs24blog/views/templates/front/'.$template.'/_local_comment.tpl')) {
                $_local_comment = 'module:cs24blog/views/templates/front/'.$template.'/_local_comment.tpl';
            } else {
            ...
            if (!file_exists(_PS_THEME_DIR_.'modules/cs24blog/views/templates/front/'.$template.'/blog.tpl') && !file_exists(_PS_MODULE_DIR_.'cs24blog/views/templates/front/'.$template.'/blog.tpl')) {
                $template = 'default';
            }
        }
        
        if (!$blog->id_cs24_blog_blog) {
            $vars = array(
                'error' => true,
            );
        ...
        $blog->preview_url = '';
        if ($blog->image) {
            $blog->image_url = $url._THEME_DIR_.'assets/img/modules/cs24blog/'.$id_shop.'/b/'.$blog->image;
//            if (!file_exists(_CS24BLOG_CACHE_IMG_DIR_.'b/'.$id_shop.'/'.$blog->id.'/lg-'.$blog->image)) {
            # FIX : CHANGE CONFIG IN BACKOFFICE NOT CHANGE AT FRONTEND
                @mkdir(_CS24BLOG_CACHE_IMG_DIR_.'b/'.$id_shop, 0777);
                @mkdir(_CS24BLOG_CACHE_IMG_DIR_.'b/'.$id_shop.'/'.$blog->id, 0777);
                    
            if (ImageManager::resize(_PS_THEME_DIR_.'assets/img/modules/cs24blog/'.$id_shop.'/b/'.$blog->image, _CS24BLOG_CACHE_IMG_DIR_.'b/'.$id_shop.'/'.$blog->id.'/lg-'.$blog->image, $image_w, $image_h)) {
                    # validate module
                    $blog->preview_url = $url._CS24BLOG_CACHE_IMG_URI_.'b/'.$id_shop.'/'.$blog->id.'/lg-'.$blog->image;
            }
//            }
        ...
        $limit = 5;
        $samecats = Cs24BlogBlog::getListBlogs($category->id_cs24_blog_cat, $this->context->language->id, 0, $limit, 'date_add', 'DESC', array('type' => 'samecat', 'id_cs24_blog_blog' => $blog->id_cs24_blog_blog), true);
        ...
        if ($blog->tags) {
            $tagrelated = Cs24BlogBlog::getListBlogs($category->id_cs24_blog_cat, $this->context->language->id, 0, $limit, 'id_cs24_blog_blog', 'DESC', array('type' => 'tag', 'tag' => $blog->tags), true);
        ...
        if ($config->get('item_comment_engine', 'local') == 'local') {
            ...
            if ($config->get('comment_engine', 'local') == 'local') {
                # validate module
                $count_comment = Cs24BlogComment::countComments($blog->id_cs24_blog_blog, true);
            }
            ...
            $comment = new Cs24BlogComment();
            $comments = $comment->getList($blog->id_cs24_blog_blog, $this->context->language->id, $p, $limit);
        ...
        $vars = array(
            ...
            'config' => $config,
            'id_cs24_blog_blog' => $blog->id_cs24_blog_blog,
        ...
        $this->setTemplate('module:cs24blog/views/templates/front/'.$template.'/blog.tpl');
    }
    ...
    public function getTemplateVarPage()
    {
        ...
        $page['canonical'] = Cs24BlogHelper::getInstance()->getBlogLink(get_object_vars($blog));
        ...
        if ($blog->image) {
            ...
            if (ImageManager::resize(_PS_THEME_DIR_.'assets/img/modules/cs24blog/'.$id_shop.'/b/'.$blog->image, _CS24BLOG_CACHE_IMG_DIR_.'b/'.$id_shop.'/'.$blog->id.'/lg-'.$blog->image, $config->get('item_img_width', 690), $config->get('item_img_height', 300))) {
                    # validate module
                    $blog->preview_url = $url._CS24BLOG_CACHE_IMG_URI_.'b/'.$id_shop.'/'.$blog->id.'/lg-'.$blog->image;
            }
            $blog->preview_url = $url._CS24BLOG_CACHE_IMG_URI_.'b/'.$id_shop.'/'.$blog->id.'/lg-'.$blog->image;
